...
static void record_proc_time(proc_id_t pid, clock_t start, clock_t end) {
    if (pid < PID_P1 || pid > PID_P2) return;
    double elapsed = clock_diff_us(start, end);
    proc_total_us[pid] += elapsed;
    proc_calls[pid]++;
}

static double compute_mem_kb(void) {
    size_t bytes = sizeof(dataset_buffer) + sizeof(uart_buf) +
                   sizeof(proc_total_us) + sizeof(proc_calls) +
                   sizeof(pcb_t) * 3;
...
static void print_metrics(clock_t start_clock, clock_t end_clock) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (proc_calls[i] > 0) {
            avg_us[i] = proc_total_us[i] / (double)proc_calls[i];
            if (avg_us[i] > max_avg) max_avg = avg_us[i];
        }
        active_us += proc_total_us[i];
    }

    double wall_us = clock_diff_us(start_clock, end_clock);
    double wall_ms = wall_us / 1000.0;
    double cpu_occupation =
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        printf("  %-3s   | %16.3f | %13.3f | %6.2f x\n",
               names[i], proc_total_us[i], avg_us[i], speedup);
    }
    printf("\nCPU Occupation: %.2f %%\n", cpu_occupation);
    printf("Mem. Occupation: %.2f KB (dataset + buffers + PCB)\n", mem_kb);
}
...
int main(int argc, char **argv) {
    ...
    for (minute_sim = 0; minute_sim < ORBIT_TIME_MIN; minute_sim += STEP_MIN) {
        ...
        record_proc_time(current, step_start, step_end);
